...
**Moussoum Defar** is the missing data layer: an open-source platform where African workers collect real data, clients launch collection campaigns, and AI models get tested against African benchmarks.
...
### Data Collection
Clients create collection campaigns (text, audio, image, video). Workers across Africa contribute data in their own languages and communities. Everyone can download and use the collected data.

### Worker System
Students, linguists, experts contribute to better AI. Scoring system, levels, gamification. The better you are, the higher you rank on the leaderboard.

### AI Evaluation
Test your AI models against African benchmarks. See if they understand Mobile Money, local languages, and culture.

### Client Dashboard
Full dashboard to manage collections, review submissions, approve/reject contributions, and download data.
...
│  - Downloads data from any collection                            │
└─────────────────────────┬───────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
│                     PLATFORM                                     │
│  - Manages collections, submissions                             │
│  - Calculates quality scores                                    │
│  - Sends notifications                                          │
│  - Tracks worker levels and leaderboard                         │
└─────────────────────────┬───────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────────┐
...
│  - Downloads data from any collection                            │
│  - Tracks score, level, and leaderboard rank                    │
└─────────────────────────────────────────────────────────────────┘
```
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/workers/auth/profile/` | GET | Get worker profile (score, level) |
| `/api/v1/workers/collections/` | GET | List all collections |
| `/api/v1/workers/collections/{id}/` | GET | Get collection details |
| `/api/v1/workers/collections/{id}/submit/` | POST | Submit data (text or file) |
| `/api/v1/workers/collections/{id}/download/` | GET | Download all files from collection (ZIP) |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/clients/profile/me/` | GET | Get client profile |
| `/api/v1/clients/data-collections/` | GET/POST | List / Create collections |
| `/api/v1/clients/submissions/` | GET | List all submissions |
| `/api/v1/clients/submissions/{id}/approve/` | POST | Approve submission |
| `/api/v1/clients/submissions/{id}/reject/` | POST | Reject submission |
| `/api/v1/clients/submissions/download-all/?collection_id={id}` | GET | Download all files from collection (ZIP) |
...
### Download Collection (Worker)

```bash
curl -O -J http://localhost:8000/api/v1/workers/collections/1/download/ \
  -H "Authorization: Bearer $WORKER_TOKEN"
```

### Download Collection (Client)

```bash
curl -O -J "http://localhost:8000/api/v1/clients/submissions/download-all/?collection_id=1" \
  -H "Authorization: Bearer $CLIENT_TOKEN"
```
...
1. **Client creates collection** → `POST /api/v1/clients/data-collections/`
2. **Worker browses collections** → `GET /api/v1/workers/collections/`
3. **Worker submits data** → `POST /api/v1/workers/collections/{id}/submit/`
4. **Client reviews submission** → `GET /api/v1/clients/submissions/`
5. **Client approves** → `POST /api/v1/clients/submissions/{id}/approve/`
6. **Worker quality score updates** → Accuracy and level recalculated
7. **Anyone downloads data** → `GET /api/v1/workers/collections/{id}/download/` or `GET /api/v1/clients/submissions/download-all/?collection_id={id}`
...
- [x] Submission approval with quality scoring
- [x] Quality scoring system
- [x] Notification system
- [x] Bulk download (ZIP) by collection
- [x] Leaderboard (top workers by quality score)
- [x] Open source - no payment system
